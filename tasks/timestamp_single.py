...
class SingleTimestampTask(BaseTask):
    ...
    def select_event(self, events: Iterable[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        if not events:
            return None
        # Ensure uniqueness by event name if provided
        seen = set()
        unique_events = []
        for ev in events:
            name = ev.get("word", None)
        ...
        if not unique_events:
            return None
        return random.choice(unique_events)
    ...
    def build_labels(self, *,
        ...
        # Count <AUDIO> token span to determine label size
        ...
        # Mask out instruction tokens up to and including assistant token
