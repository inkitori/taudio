...
class AllTimestampsTask(BaseTask):
    ...
    def calculate_loss(self, audio_logits, audio_labels, audio_labels_frame_mask, model_adapter: BaseModelAdapter, use_poisson_loss: bool, class_weighting: bool) -> torch.Tensor:
        ...
        counts = first_audio_labels.sum().item()
        ...
        if use_poisson_loss:
            ...
            # logging.info("Audio Labels: {audio_labels}")
            all_timestamps = infer_timestamps(counts, first_audio_logits.cpu().float().detach().numpy())
            all_timestamps = all_timestamps / model_adapter.seconds_to_embedding
            logging.info(f"All Predicted Timestamps: {all_timestamps}")
        else:
