...
import os
from utils.utils import round_timestamp_python, remove_indices
    ...
    for video_id, group in events_df.groupby('id'):
        ...
        for _, row in group.iterrows():
            event = {
                ...
                'start': round_timestamp_python(row['label_start'] - row['start_time']),
                'end': round_timestamp_python(row['label_end'] - row['start_time'])
            }
            events_list.append(event)
        events_by_video_id[video_id] = events_list

    # 4. Load the specified split from the Hugging Face dataset
    original_dataset_split = load_dataset(dataset_name, split=split)
    original_dataset_split = remove_indices(original_dataset_split, train_exclude_indices if split == 'train' else eval_exclude_indices)
...
if __name__ == '__main__':
    ...
    # --- Configuration ---
    TRAIN_CSV_PATH = 'dataset_scripts/merged_all_train_human_sounds.csv'
    EVAL_CSV_PATH = 'dataset_scripts/merged_eval_human_sounds.csv'
    DATASET_NAME = 'enyoukai/AudioSet-Strong'
    # IMPORTANT: Replace with your Hugging Face username and desired dataset name.
    HF_REPO_NAME = "enyoukai/AudioSet-Strong-Human-Sounds"
    ...
    eval_split = create_dataset_split(
        ...
        dataset_name=DATASET_NAME,
        split='eval' # Using the 'test' split for evaluation
    )
