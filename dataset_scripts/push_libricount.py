...
This script reads the `mixtures_metadata.json` files from train and test
directories and constructs a Hugging Face dataset with the following schema:
...
The resulting dataset with train and test splits is uploaded to the Hub
repository specified by `--repo-id` (defaults to `enyoukai/libricount-timings`).
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--train-dir",
        type=Path,
        default=default_root / "mixtures_start_random",
        help="Path to the directory containing train mixtures_metadata.json file.",
    )
    parser.add_argument(
        "--test-dir",
        type=Path,
        default=default_root / "mixtures_start_random_test",
        help="Path to the directory containing test mixtures_metadata.json file.",
    )
    parser.add_argument(
        "--audio-root",
        type=Path,
...
def build_dataset(mixture_dir: Path, audio_root: Path) -> Dataset:
    metadata_path = mixture_dir / "mixtures_metadata.json"
...
def push_dataset(dataset_dict: DatasetDict, repo_id: str, max_shard_size: str, token: str | None) -> None:
...
def main() -> None:
    ...
    print(f"Building train dataset from {args.train_dir}")
    train_dataset = build_dataset(args.train_dir, args.audio_root)
    
    print(f"Building test dataset from {args.test_dir}")
    test_dataset = build_dataset(args.test_dir, args.audio_root)
    ...
    push_dataset(dataset_dict, args.repo_id, args.max_shard_size, args.token)
