...
class TAudio(nn.Module):
    def __init__(
        ...
        self.model_adapter = create_adapter(model_id=model_id, bidirectional_audio=bidirectional_audio, dtype=dtype, scaling_factor=scaling_factor)
        ...
        self.linear = nn.Linear(self.hidden_dim, scaling_factor, dtype=self.model_adapter.dtype)
    ...
    def generate(self, **kwargs):
        with torch.no_grad():
            tokens = self.model_adapter.generate(**kwargs, eos_token_id=self.model_adapter.processor.tokenizer.eos_token_id, max_new_tokens=1024)
    ...
    def generate_batch(self, **kwargs):
        with torch.no_grad():
            return self.model_adapter.generate_batch(**kwargs, eos_token_id=self.model_adapter.processor.tokenizer.eos_token_id, decode_tokens=True)
